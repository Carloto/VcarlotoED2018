...
struct tmp_fileArguments {
    ...
    char *input_via; // Argumento do comando - via
    ...
    char *inputViaFileName; // Nome completo do arquivo .via
...
fileArguments *createFileArguments() {
    ...
    create_struct->inputViaFileName = NULL;
...
void killFileArguments(fileArguments **kill_struct) {
    ...
    freeString(&(*kill_struct)->input_via);
    ...
    freeString(&(*kill_struct)->inputViaFileName);
...
void printInputArguments(fileArguments *print_struct) {
    ...
    printThis(print_struct->input_via);
    ...
    printThis(print_struct->inputViaFileName);
...
void setFileArguments(fileArguments **set_struct, int argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (strcmp("-via", argv[i]) == 0) {
            i++;
            copyString(&(*set_struct)->input_via, argv[i]);
        }
    ...
    // Verificar se existe via
    if ((*set_struct)->input_via != NULL) {
        // Verificar se -via possui "." e concatenar nome do arquivo via
        if ((*set_struct)->input_via != NULL) {
            if ((*set_struct)->input_via[0] == '.') {
                removeFirstChar(&(*set_struct)->input_via);
            }
            strcatFileName((&(*set_struct)->inputViaFileName), (*set_struct)->input_e, &(*set_struct)->input_via,
                           "\0");
        } else {
            copyString(&(*set_struct)->inputViaFileName, (*set_struct)->input_via);
        }
    }
...
char *getInputViaFileName(fileArguments *tmpStructs) {
    return tmpStructs->inputViaFileName;
}
